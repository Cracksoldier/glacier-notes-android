...
M00 through M13 are complete. The design system, app shell, branding, English/German localization, persisted settings, the domain models, the SQLite layer (schema, migrations, transactions), the repositories on top of it, the note list and Markdown editor, notebook management, labels/colors/pinning/archive/trash, checklist notes, image attachments, local search with sorting, and the full `.glacier.json` exchange contract in both directions are in place. Notes can be created, edited, rendered, filtered by notebook and label, coloured, pinned, labelled, archived, trashed, restored and purged end to end; a note can be a Markdown note or an ordered checklist and can be converted either way; a Markdown note can carry images picked from the system picker, stored app-privately and garbage-collected when nothing references them; titles, bodies and checklist items are searchable within any scope with the matches highlighted, and the list is sortable three ways; and the whole collection round-trips through a file the desktop app reads and writes unchanged, with a preview, both conflict strategies and an all-or-nothing rollback. Every route now renders a real page. Work continues at M14.
...
| Export destination | app-private `Directory.Data`, no picker and no share sheet | M14 |
| Import source | a plain `<input type="file">`, not a document picker | M14 |
...
Resolved by M12: `docs/import-export.md` records what "ported verbatim" from the desktop's `transfer-core.ts` excludes and why, why all three export scopes ship although only `all` is wired, why the export note set is not `list({kind:'all'})` and why the trash union beat a seventh `NoteScope`, `CollectionRepository.snapshot()` and the one-turn rule, `ImageFileStore.read()`'s encoding trap, why a missing image aborts here but not on the desktop, why streaming was rejected, that `Directory.Data` is a harness rather than M14's destination, and how the desktop fixture is regenerated. **Read it before touching `src/app/core/import-export/` or changing what an export contains.** Four rules that will otherwise be broken silently: the export note set is the union of `{kind:'all'}` and `{kind:'trashed'}`, since `{kind:'all'}` is `deleted_at IS NULL` and would silently drop the trash from every backup; `readCollectionSnapshot` composes query free functions inside one `read()` and must never re-enter the queue, or a concurrent write tears the export; nothing may reach storage before `validateEnvelope` passes, and no catch block may log its error, because a filesystem message carries a path and a database one carries bound note text; and the key *set* is the contract while key *order* is not — the desktop's own `NoteRepo.update` appends optional keys set after creation, so a byte-comparison against a desktop file will fail for reasons that mean nothing.
...
Under `src/app/core/`, `localization/`, `preferences/`, `models/`, `database/`, `repositories/`, `markdown/`, `images/`, `filesystem/` and `import-export/` hold code; `native/` is still an empty placeholder. UI code must reach persistence only through the repository services in `core/repositories` — never via direct SQLite plugin calls, and never by injecting `DATABASE_ADAPTER` outside `core/database` and `core/repositories`.
