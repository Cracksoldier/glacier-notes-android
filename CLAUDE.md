...
M00 through M09 are complete. The design system, app shell, branding, English/German localization, persisted settings, the domain models, the SQLite layer (schema, migrations, transactions), the repositories on top of it, the note list and Markdown editor, notebook management, labels/colors/pinning/archive/trash, and checklist notes are in place. Notes can be created, edited, rendered, filtered by notebook and label, coloured, pinned, labelled, archived, trashed, restored and purged end to end; a note can be a Markdown note or an ordered checklist and can be converted either way; only import/export still routes to a placeholder page. Work continues at M10.
